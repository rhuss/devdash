# Brainstorm Overview

Last updated: 2026-09-08

## Sessions

| # | Date | Topic | Status | Spec | Issue |
|---|------|-------|--------|------|-------|
| 01 | 2026-09-08 | devdash-tui | active | - | - |

## Open Threads

- Behaviour when `gh` is missing, the token is absent or expired, or the API rate limit is exhausted (from #01)
- Configuration file location and format (from #01)
- Sort order for the repository list and the pull request list (from #01)
- Rendering a repository with no CI configured versus one that is genuinely failing (from #01)
- Rate-limit budget for interval refresh, and the default interval (from #01)
- Whether the repository CI glyph reflects all open PRs or only those matching the active filter (from #01)

## Parked Ideas

None.
